...
class StopButton : public BWidgets::Button
{
public:
	StopButton () : StopButton (0.0, 0.0, BWIDGETS_DEFAULT_BUTTON_WIDTH, BWIDGETS_DEFAULT_BUTTON_HEIGHT, "button", 0.0) {}

	StopButton (const double x, const double y, const double width, const double height, const std::string& name, double defaultValue = 0.0) :
			Button (x, y, width, height, name, defaultValue) {}

	virtual Widget* clone () const override {return new StopButton (*this);}
	...
	virtual void draw (const BUtilities::RectArea& area) override
	{
		...
		if ((getWidth () >= 6) && (getHeight () >= 6))
		{
			...
			if (cairo_status (cr) == CAIRO_STATUS_SUCCESS)
			{
				...
				cairo_move_to (cr, x0 + w/2 - size/4, y0 + h/2 - size/4);
				cairo_line_to (cr, x0 + w/2 + size/4, y0 + h/2 - size/4);
				cairo_line_to (cr, x0 + w/2 + size/4, y0 + h/2 + size/4);
				cairo_line_to (cr, x0 + w/2 - size/4, y0 + h/2 + size/4);
				cairo_close_path (cr);
